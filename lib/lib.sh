...
# Premium color scheme - Fire & Gold theme
export COLOR_FIRE_RED='\033[38;5;196m'      # Deep fire red
export COLOR_FIRE_ORANGE='\033[38;5;202m'   # Fire orange
export COLOR_GOLD='\033[38;5;220m'          # Rich gold
export COLOR_LIGHT_GOLD='\033[38;5;228m'    # Light gold
export COLOR_WHITE='\033[1;37m'             # Bright white
export COLOR_GRAY='\033[90m'                # Dark gray
export COLOR_LIGHT_GRAY='\033[37m'          # Light gray
export COLOR_PURPLE='\033[38;5;141m'        # Soft purple
export COLOR_PINK='\033[38;5;205m'          # Pink accent
export COLOR_TEAL='\033[38;5;45m'           # Teal accent
export COLOR_LIME='\033[38;5;118m'          # Lime green

# Background colors
export BG_ORANGE='\033[48;5;208m'
export BG_RED='\033[48;5;196m'
export BG_GREEN='\033[48;5;28m'
export BG_BLUE='\033[48;5;27m'

# Text formatting
export TEXT_BOLD='\033[1m'
export TEXT_DIM='\033[2m'
export TEXT_ITALIC='\033[3m'
...
# Emojis (using ASCII alternatives for compatibility)
export EMOJI_CHECK='✓'
export EMOJI_CROSS='✗'
export EMOJI_ARROW='→'
export EMOJI_BULLET='•'
...
# Draw a beautiful horizontal line with flame gradient
print_flame_line() {
  local width="${1:-70}"
  local char="${2:-─}"
  local line=""
  ...
  echo -e "${COLOR_FIRE_ORANGE}${line}${COLOR_NC}"
}
...
print_box() {
  ...
  if [ $extra_padding -eq 1 ]; then
    right_pad+="═"
  fi
  
  echo ""
  echo -e "${COLOR_FIRE_ORANGE}  ╭${left_pad} ${COLOR_GOLD}${TEXT_BOLD}${title}${TEXT_BOLD} ${COLOR_FIRE_ORANGE}${right_pad}╮${COLOR_NC}"
  echo ""
}

# Close the decorative box
print_box_close() {
  local width="${1:-70}"
  local line=""
...
# Print a section header with flame styling
print_section() {
  local title="$1"
  local icon="${2:-$FIRE}"
  ...
  echo -e "  ${COLOR_FIRE_ORANGE}│${COLOR_NC}  ${COLOR_GOLD}${icon}${COLOR_NC}  ${TEXT_BOLD}${COLOR_WHITE}${title}${COLOR_NC}"
  echo -e "  ${COLOR_FIRE_ORANGE}╰────────────────────────────────────────────────────────────────────╯${COLOR_NC}"
  echo ""
}

# Print a step indicator (e.g., "Step 1 of 5")
print_step() {
  local current="$1"
  local total="$2"
  local label="$3"
  ...
  echo ""
  echo -e "  ${COLOR_FIRE_ORANGE}┌────────────────────────────────────────────────────────────────────┐${COLOR_NC}"
  echo -e "  ${COLOR_FIRE_ORANGE}│${COLOR_NC} ${COLOR_GOLD}Step ${current} of ${total}${COLOR_NC} ${COLOR_GRAY}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${COLOR_NC} ${COLOR_GOLD}${label}${COLOR_NC}"
  echo -e "  ${COLOR_FIRE_ORANGE}└────────────────────────────────────────────────────────────────────┘${COLOR_NC}"
  echo ""
}
...
spinner() {
  ...
  while [ "$(ps a | awk '{print $1}' | grep -w $pid)" ]; do
    local temp=${spinstr#?}
    printf "  ${COLOR_FIRE_ORANGE}%c${COLOR_NC} ${COLOR_SOFT_WHITE}%s${COLOR_NC}" "$spinstr" "$message"
...
progress_bar() {
  ...
  for ((i=0; i<filled; i++)); do
    if [ $i -lt $((width / 3)) ]; then
      bar+="${COLOR_FIRE_RED}█${COLOR_NC}"
    elif [ $i -lt $((width * 2 / 3)) ]; then
      bar+="${COLOR_FIRE_ORANGE}█${COLOR_NC}"
    else
      bar+="${COLOR_GOLD}█${COLOR_NC}"
    fi
  done
  
  # Build the empty portion
  for ((i=0; i<empty; i++)); do
    bar+="${COLOR_DARK_GRAY}░${COLOR_NC}"
  done
  
  printf "\r  ${COLOR_FIRE_ORANGE}[${COLOR_NC}%s${COLOR_FIRE_ORANGE}]${COLOR_NC} ${COLOR_GOLD}%3d%%${COLOR_NC}" "$bar" "$percentage"
}

# Enhanced output functions with icons
output_info() {
  echo -e "  ${COLOR_SKY}${INFO}${COLOR_NC} ${COLOR_SOFT_WHITE}$1${COLOR_NC}"
}

output_success() {
  echo ""
  echo -e "  ${COLOR_LIME}${CHECK_MARK}${COLOR_NC} ${COLOR_GREEN}${TEXT_BOLD}SUCCESS${TEXT_BOLD}${COLOR_NC}: ${COLOR_WHITE}$1${COLOR_NC}"
  echo ""
}

output_error() {
  echo ""
  echo -e "  ${COLOR_RED}${X_MARK}${COLOR_NC} ${COLOR_RED}${TEXT_BOLD}ERROR${TEXT_BOLD}${COLOR_NC}: ${COLOR_WHITE}$1${COLOR_NC}" >&2
  echo ""
}

output_warning() {
  echo ""
  echo -e "  ${COLOR_GOLD}${WARNING}${COLOR_NC} ${COLOR_YELLOW}${TEXT_BOLD}WARNING${TEXT_BOLD}${COLOR_NC}: ${COLOR_WHITE}$1${COLOR_NC}"
  echo ""
}

output_prompt() {
  echo -e "  ${COLOR_GOLD}${ARROW_RIGHT}${COLOR_NC} ${COLOR_WHITE}$1${COLOR_NC}"
}
...
print_kv() {
  ...
  printf "  ${COLOR_FIRE_ORANGE}%-${key_width}s${COLOR_NC} ${COLOR_GOLD}→${COLOR_NC} ${COLOR_WHITE}%s${COLOR_NC}\n" "$key" "$value"
}
...
print_menu_item() {
  ...
  if [ "$selected" == "true" ]; then
    echo -e "  ${COLOR_GOLD}${ARROW_RIGHT}${COLOR_NC} ${COLOR_GOLD}[${number}]${COLOR_NC} ${COLOR_WHITE}${TEXT_BOLD}${label}${TEXT_BOLD}${COLOR_NC}"
  else
    echo -e "  ${COLOR_GRAY}${BULLET}${COLOR_NC} ${COLOR_FIRE_ORANGE}[${number}]${COLOR_NC} ${COLOR_SOFT_WHITE}${label}${COLOR_NC}"
  fi
  
  if [ -n "$description" ]; then
    echo -e "      ${COLOR_GRAY}${description}${COLOR_NC}"
  fi
}

# Print status indicator (installed/not installed)
print_status() {
  local label="$1"
  local installed="$2"
  local version="${3:-}"
  ...
  if [ "$installed" == "true" ]; then
    if [ -n "$version" ]; then
      echo -e "  ${COLOR_LIME}${CHECK_MARK}${COLOR_NC} ${COLOR_SOFT_WHITE}${label}${COLOR_NC} ${COLOR_GRAY}(${version})${COLOR_NC}"
    else
      echo -e "  ${COLOR_LIME}${CHECK_MARK}${COLOR_NC} ${COLOR_SOFT_WHITE}${label}${COLOR_NC}"
    fi
  else
    echo -e "  ${COLOR_RED}${X_MARK}${COLOR_NC} ${COLOR_GRAY}${label}${COLOR_NC}"
  fi
}

# Animated flame text effect
animate_flame() {
  local text="$1"
  local delay="${2:-0.05}"
  ...
  for ((i=0; i<${#text}; i++)); do
    ...
    printf "%b%s%b" "$color" "$char" "$COLOR_NC"
    sleep $delay
  done
  echo ""
}

# Print a decorative separator with flame effect
print_separator() {
  echo ""
  echo -e "  ${COLOR_FIRE_RED}▓▓▓${COLOR_FIRE_ORANGE}▓▓▓${COLOR_ORANGE}▓▓▓${COLOR_GOLD}▓▓▓${COLOR_YELLOW}▓▓▓${COLOR_NC}"
  echo ""
}
...
confirm_dialog() {
  ...
  local prompt_text=""
  if [ "$default" == "y" ]; then
    prompt_text="[${COLOR_LIME}Y${COLOR_NC}/${COLOR_RED}n${COLOR_NC}]"
  else
    prompt_text="[${COLOR_LIME}y${COLOR_NC}/${COLOR_RED}N${COLOR_NC}]"
  fi
  
  echo -e "  ${COLOR_GOLD}${QUESTION}${COLOR_NC} ${COLOR_WHITE}${message}${COLOR_NC} ${prompt_text}: \c"
}

# Print a summary box with configuration
print_summary_box() {
  print_box "Configuration Summary"
  
  # Configuration items would be printed here by the calling code
